pub mod radix_tree_operations {
    ...
    fn prefix_check(curr_str: &String, check_str: &String) -> usize {
        ...
        while ptr < curr_str.len()
            && ptr < check_str.len()
            && curr_str_bytes[ptr] == check_str_bytes[ptr]
        {
            ptr += 1
        }
    ...
    pub fn radix_tree_lookup(root: Node, value: String) -> bool {
        ...
        if result == 0
            && current_node.current_value.is_empty()
    ...
    pub fn radix_tree_insert(root: Node, value: String) -> Node {
        ...
        // just handling inserting into the root node first
        ...
        let mut remainder: String = split_values.1.to_string();

        while !remain_value.is_empty() {
